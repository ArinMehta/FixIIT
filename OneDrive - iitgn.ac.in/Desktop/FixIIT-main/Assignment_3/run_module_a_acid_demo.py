...
def main():
    ...
    try:
        assign_ticket_transaction(db, assignment_id=10, ticket_id=2, tech_id=17, admin_id=28, fail_after_ops=2)
    ...
    # Verify atomicity: ticket 2 status should NOT have changed to 2 (should still be 2 but with no assignment)
    ticket_2_after = [t for t in state_after_failure["tickets"] if t["ticket_id"] == 2][0]
    assignments_after = state_after_failure["assignments"]
    if ticket_2_after["status_id"] == 2 and not any(a["ticket_id"] == 2 for a in assignments_after):
    ...
    assign_ticket_transaction(db, assignment_id=3, ticket_id=3, tech_id=17, admin_id=28)
